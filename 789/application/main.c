...
//    onetimer = xTimerCreate((const char *)"onetimer", mainSOFTWARE_TIMER_PERIOD_MS,
//            pdFALSE, (void *)0, onetimerTimerCallback);
...
void start_task1(void *pvParameters)
{
    ...
    uint8_t remain_size;
    countSemaphore = xSemaphoreCreateCounting(255,20);//创建计数型信号量
    ...
    timer_config();
//    eclic_global_interrupt_enable();
//    eclic_set_nlbits(ECLIC_GROUP_LEVEL7_PRIO0);
//    eclic_irq_enable(TIMER2_IRQn,7,0);
//      timer2_config();
	gd_rvstar_led_init(LED3);
	gd_rvstar_led_init(LED1);
	gd_rvstar_led_init(LED2);
	Msg_Queue =xQueueCreate(30,30*sizeof(uint8_t));
    ...
    while (1) {
    	...
    	if(gpio_input_bit_get(GPIOA,BIT(5) ))
    	{
    		...
    		xQueueSend(Msg_Queue,sendbuf,3);
    		msgq_remain_size = uxQueueSpacesAvailable(Msg_Queue);
    		msgq_total_size = uxQueueSpacesAvailable(Msg_Queue)+uxQueueMessagesWaiting(Msg_Queue);

    		printf("remain_size = %d\n",msgq_remain_size);
    		printf("total_size = %d\n",msgq_total_size);
    		 xQueuePeek(Msg_Queue,queuebuf,0);
    	}
...
void start_task2(void *pvParameters)
{
    ...
    while (1) {
    	...
    	if(num ==6)
    	{
    		//vTaskDelete(StartTask1_Handler);
    		//vTaskDelete(StartTask2_Handler);
    	}
//
//    	{
//
...
//
//    			//printf("get the BinarySemaphore!\n");
//    		}
...
//
//    		{
//    			if(countSemaphore!=NULL)/*if create successfully*/
//    			{
//    				if(gd_key_state_get(KEY_WAKEUP)==1)
//    				{
//    					err= xSemaphoreGive(countSemaphore);
//    					if(err == pdFALSE)
//    					{
//    						printf("signal release failed\n");
//    					}
//    					semavalue= uxSemaphoreGetCount(countSemaphore);
//    					printf("semavalue = %d\n",semavalue);
//    				}
...
//
//    				(EventBits_t)EVENTBIT_ALL,(BaseType_t)pdTRUE,(BaseType_t)pdTRUE,(TickType_t)portMAX_DELAY);
//
//    		printf("Eventvalue:%d\r\n",EventValue);
//    	}
    	printf("task2 is running %d.....\r\n", cnt++);
    	// xQueueSend(Msg_Queue,&a[1],0);
    	//delay();
    	//vTaskDelayUntil(20,1);
       vTaskDelay(100);
    }
}
...
//		niubi = xEventGroupGetBits(EventGroupHandler);
//		    	printf("%d\n",niubi);
//		if(gpio_input_bit_get(GPIOA,BIT(5) ))
//		{
//			gd_led_on(LED1);
...
//
//		}
//		if(gpio_input_bit_get(GPIOA,BIT(6) ))
//		{
//			gd_led_on(LED2);
//			vTaskResume(StartTask1_Handler);
//			printf("Recovery task 1\n");
//
//		}
