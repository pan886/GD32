...
void start_task2(void *pvParameters)
{
    ...
    while (1) {
    	...
    	xTaskNotifyWait((uint32_t)0,(uint32_t)0xffffffff,(uint32_t* )&NotifyValue,portMAX_DELAY);

    	if(NotifyValue == (EVENTBIT_1|EVENTBIT_2))
    		printf("i get the event!\n");
...
void key_task(void *pvParameters)
{
	...
	while(1)
	{
			...
			if(gpio_input_bit_get(GPIOA,BIT(5) ))
		    	{
		    		xTaskNotify(StartTask2_Handler,EVENTBIT_2,eSetBits);
...
void EXTI0_IRQHandler()
{
    ...
    if (SET == exti_interrupt_flag_get(WAKEUP_KEY_PIN)){
        ...
        if(RESET == gd_rvstar_key_state_get(KEY_WAKEUP))//io input state
        {
          ...
          xTaskNotifyFromISR(StartTask2_Handler,EVENTBIT_1,eSetBits,xHigherPriorityTaskWoken); //xEventGroupSetBitsFromISR(EventGroupHandler,EVENTBIT_1,&xHigherPriorityTaskWoken); /* set led to BLUE */
