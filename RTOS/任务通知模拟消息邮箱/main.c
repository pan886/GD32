...
void start_task1(void *pvParameters)
{
    ...
    while (1) {
		...
		xTaskNotify((TaskHandle_t)StartTask2_Handler,key,(eNotifyAction)eSetValueWithOverwrite);
    	if(ulTaskNotifyTake(pdTRUE,portMAX_DELAY)== 1)
    	{
    		printf("i get the notify!\n");
    	}
    ...
    if (SET == exti_interrupt_flag_get(WAKEUP_KEY_PIN)){
        ...
        if(RESET == gd_rvstar_key_state_get(KEY_WAKEUP))//io input state
        {
           ...
           // xSemaphoreGiveFromISR(BinarySemaphore,&xHigherPriorityTaskWoken);
            vTaskNotifyGiveFromISR(StartTask1_Handler,&xHigherPriorityTaskWoken);
